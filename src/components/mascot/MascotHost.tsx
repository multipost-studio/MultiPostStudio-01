 ...
 *  3. Route-aware contextual hints — once per section per session.
 */
export function MascotHost({ variant = "full" }: { variant?: "full" | "subtle" }) {
  const pathname = usePathname() ?? "";
  const router = useRouter();

  const [mounted, setMounted] = React.useState(false);
  const [prefs, setPrefs] = React.useState<MascotPrefs>({ muted: false, hidden: false, tourDone: false });
  ...
  // surface as companion feedback, reusing the toast's own copy.
  React.useEffect(() => {
    if (variant !== "full") return;
  ...
  const panel = assistantOpen ? (
    ...
    <MascotBubble key="bubble" message={message} onClose={hideMessage} />
  ) : null;
  ...
  return (
    ...
    >
      ...
      <MultiPostMascot onActivate={() => (assistantOpen ? setAssistantOpen(false) : openAssistant())} />
    </div>
  );
}
